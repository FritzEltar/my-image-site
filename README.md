...
你的原图很大（最大一张 6.6 MB / 4096×3072）。生成脚本会**顺手做两张小图**，
原图本身不再被页面自动加载，只用于「下载」和「复制图片 URL」：

```
images/全部/大图集1/character1/大图.png   6.6 MB   ← 原图，只在点「下载」时用
views/ 全部/大图集1/character1/大图.jpg    300 KB   ← 灯箱查看用（最长边 1600px）
thumbs/全部/大图集1/character1/大图.jpg     50 KB   ← 网格 / 封面用（最长边 600px）
```

### 为什么要有 `views/` 这一档

之前只做了 `thumbs/`，结果**点开大图比优化前还慢**。原因是：

- 优化前网格直接加载原图，你点开时图已经在浏览器缓存里 → 秒开
- 优化后网格只加载 50KB 缩略图，点开才第一次去下 5～7MB 原图 → 要等

更关键的是**灯箱最多只显示约 1000px 宽**（`max-height: 65vh` + `max-width: 1000px`），
却下载 4096px 的图 —— 白白多下 **130 倍**的流量。

所以现在灯箱改用 `views/`（1600px，比显示尺寸大 1.6 倍，高清屏也够清晰）：

| 图片 | 原图 | 查看图（灯箱） | 缩略图（网格） |
|---|---|---|---|
| 036303AB….jpg | 6,764 KB | **300 KB**（省 23 倍） | 50 KB |
| 屏幕截图(37).png | 5,684 KB | **273 KB**（省 21 倍） | 48 KB |
| 屏幕截图(48).png | 4,109 KB | **85 KB**（省 48 倍） | 18 KB |

另外灯箱做了两件事让等待感消失：

- **秒显缩略图占位**：点开瞬间先显示放大并轻微模糊的缩略图，
  右上角提示「载入大图…」，大图下载完自动换上清晰版，不会对着白框干等
...
- `images.json` 是三级嵌套清单：`groups → collections → roles → images`，
  每个图片条目带 `file`（原图）、`view`（灯箱用）、`thumb`（网格用）三个路径。
